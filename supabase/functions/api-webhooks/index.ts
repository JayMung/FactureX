/**
 * API Endpoint: /v1/api-webhooks
 * Manage webhooks for event notifications (Discord, n8n, Slack, etc.)
 * 
 * Versioned routes:
 *   /v1/api-webhooks   — current (recommended)
 *   /api-webhooks       — legacy (deprecated, supported until 2026-06-01)
 */
...
const FUNCTION_NAME = 'api-webhooks';
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const versionMetaPost = getVersionMeta(req, FUNCTION_NAME);
      const response = successResponse(
        { webhook: data },
        {
          organization_id: keyData.organization_id,
    ...
    if (req.method === 'PUT') {
      ...
      const versionMetaPut = getVersionMeta(req, FUNCTION_NAME);
      const response = successResponse(
        { webhook: data },
        {
          organization_id: keyData.organization_id,
    ...
    if (req.method === 'DELETE') {
      ...
      const versionMetaDel = getVersionMeta(req, FUNCTION_NAME);
      const response = successResponse(
