-- Migration: Create paiements system for factures
-- Fixed to use correct column names from factures table
...
BEGIN
  UPDATE factures
  SET solde_restant = total_general - COALESCE(montant_paye, 0)
...
BEGIN
  IF NEW.type_paiement = 'facture' THEN
    v_description := 'Paiement facture ' || COALESCE((
      SELECT facture_number FROM factures WHERE id = NEW.facture_id LIMIT 1
    ), 'N/A');
  ELSE
...
COMMENT ON TABLE paiements IS 'Unified payment tracking for factures. Automatically creates revenue transactions and updates account balances.';
